...
/**
 * 定义一个结构体，用于表示 Spice 会话的连接信息。
 * 
 * 结构体成员：
 * - connection_id: 连接的唯一标识符。
 * - num_channel_caps: 通道能力的数量。
 * - destport: 目标端口号。
 * - client_auth: 客户端认证信息。
 * - server_auth: 服务器认证信息。
 * - auth_selected: 已选择的认证方式。
 * - next_state: 下一个会话状态。
 * - playback_mode: 回放模式。
 * - channel_type: 通道类型。
 * - channel_id: 通道标识符。
 * - client_mini_header: 客户端是否使用迷你头。
 * - server_mini_header: 服务器是否使用迷你头。
 */
typedef struct {
    uint32_t connection_id;        // 连接的唯一标识符
    uint32_t num_channel_caps;     // 通道能力的数量
    uint32_t destport;             // 目标端口号
    uint32_t client_auth;          // 客户端认证信息
    uint32_t server_auth;          // 服务器认证信息
    uint32_t auth_selected;        // 已选择的认证方式
    spice_session_state_e next_state; // 下一个会话状态
    uint16_t playback_mode;        // 回放模式
    uint8_t  channel_type;         // 通道类型
    uint8_t  channel_id;           // 通道标识符
    bool client_mini_header;       // 客户端是否使用迷你头
    bool server_mini_header;       // 服务器是否使用迷你头
